...
def train(epoch_idx,mAP):
    ...
    for batch_idx, (vxi,vxj,axi, axj) in enumerate(train_loader):
        ...
        aij = torch.cat((aoi, aoj), dim=-1).cuda().view(-1,feature_dim*2).contiguous()
            ...
            # aij = torch.cat((aoi, aoj), dim=-1).cuda().view(-1,feature_dim*2).contiguous()
        w1 = h(attij).view(-1,args.num_per_group)
        w2 = h(vij).view(-1,args.num_per_group)
        w3 = h(aij).view(-1,args.num_per_group)

        attloss = limloss(attouti, attoutj, w1)
        vloss = limloss(vouti, voutj, w2)
        # print(vouti,voutj)
        aloss = limloss(aouti, aoutj, w3)
