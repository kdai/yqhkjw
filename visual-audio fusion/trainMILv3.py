...
def train(epoch_idx,mAP):
    ...
    for batch_idx, (posv,posa,negv,nega,pos_label,neg_label) in enumerate(train_loader):
        ...
        # rloss = RKLoss(bag_predict[:b1],bag_predict[b1:])
        ...
        # recoder.update('rloss',rloss.item(),epoch_idx*len(train_loader))
...
if __name__ == '__main__':
    ...
    AHLoss = AdaptiveHingerLossv2().to(device)

    RKLoss = Rankingloss().to(device)
